...
const sidebarVariants = cva(
  "fixed inset-y-0 left-0 z-20 flex w-64 flex-col border-r bg-sidebar-background text-sidebar-foreground transition-transform duration-300 ease-in-out md:relative",
  {
    variants: {
      isOpen: {
        true: "translate-x-0",
...
export function Sidebar({ className, children, ...props }: SidebarProps) {
  ...
  return (
    <aside className={cn(sidebarVariants({ isOpen }), className)} {...props}>
      {children}
    </aside>
  )
}
...
export function SidebarFooter({ className, children, ...props }: SidebarFooterProps) {
  ...
  return (
    <div
      className={cn("flex items-center border-t p-2", isOpen ? "justify-between" : "justify-center", className)}
      {...props}
    >
      {isOpen ? children : null}
    </div>
